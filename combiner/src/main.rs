...
impl FloatingImage {
    ...
    fn set_data(&mut self, data: Vec<u8>) -> Result<(), ImageDataError> {
        if data.len() > self.data.capacity() {
            Err(ImageDataError::BufferTooSmall)
        } else {
            self.data = data;
            Ok(())
        }
...
fn main() -> Result<(), ImageDataError> {
    ...
    if let Err(e) = image::save_buffer_with_format(output.name, &output.data, output.width, output.height, image::ColorType::Rgb8, image_format_1) {
        Err(ImageDataError::FailedToSaveImage(e))
    } else {
        Ok(())
    }    
}
        ...
        Ok(image_reader) => {
            if let Some(image_format) = image_reader.format() {
                match image_reader.decode() {
                    ...
                    Err(e) => Err(ImageDataError::FailedToDecodeImage(e))
                }
            } else {
                 Err(ImageDataError::FailedToReadImageFormat)
            }
        }
        Err(_) => Err(ImageDataError::CannotOpenFile(path))
    }
}
